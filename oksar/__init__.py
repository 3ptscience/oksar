...
class Forward(object):
    ...
    #          % calculates station geometry constants for finite source
                ...
                ret = c2_r + dc_et
                if np.any(ret < 1e-14):
                    raise Exception('dccon2 b %f %f %f %f'%(ret,c2_r,dc_et,dc_q,dc_xi))
                ...
                ind = (dc_et < F0) & (dc_q == F0) & (dc_xi == F0)
                    ...
                    # if((c2_r-dc_et) < 1e-14):
                    #     raise Exception('dccon2 a %f %f %f %f %f'% (c2_3-dc_et, c2_r, dc_et, dc_q, dc_xi))
    ...
    #          %  part B of displacement and strain at depth due to buried fauls in semi-infinite medium
                rd = c2_r + c2_d
                if np.any(rd < 1e-14):
                    raise Exception('ub %f %f %f %f %f %f'%(rd ,c2_r, c2_d,xi,et,q))
                ...
                if(c0_cd !=F0):
                    ...
                    ai4 = F1/c0_cdcd * (xi/rd*c0_sdcd + F2*np.arctan((et*(xx+q*c0_cd) + xx*(c2_r+xx)*c0_sd) / (xi*(c2_r+xx)*c0_cd)) )
                    ai4[xi==F0] = F0
...
class InSarModel(object):
    ...
    center = Vector(773728.2977967655,4223586.816611591,0)
    depthT=0
    depthB=15000
    wv=0.028333
    ...
    LOS = Vector(0.3825, 0.0780, 0.9205)
