...
def getLOSvector(utmLoc, utmZone, refPoint, satAltitude, satAzimuth, satIncidence, earthRadius):
    #calculate beta - the angle at earth centre between reference point and satellite nadir
    DEG2RAD = np.pi/180.
    ...
    origy, origx = utm.to_latlon(refPoint.x, refPoint.y, locationUTMzone, northern=locationUTMzone>0)

    xy = np.array([utm.to_latlon(u[0],   u[1],   locationUTMzone, northern=locationUTMzone>0) for u in utmLoc])
    y = xy[:,0]
    x = xy[:,1]

    angdist = ang_to_gc(x,y,origx,origy,satAzimuth)

    # calculate beta2, the angle at earth center between roaming point and satellite nadir track, assuming right-looking satellite
    beta2 = beta-angdist
    beta2 = beta2*DEG2RAD
    ...
    return Vector(los_x, los_y, los_z)


def ang_to_gc( x, y, origx,  origy,  satAzimuth):
    """function to calculate angular distance to great circle passing through given point"""
    ...
    Ngc[0] = (np.sin(satAzimuth)*np.sin(origy)*np.cos(origx)) - (np.cos(satAzimuth)*np.sin(origx))
    Ngc[1] = (np.sin(satAzimuth)*np.sin(origy)*np.sin(origx)) + (np.cos(satAzimuth)*np.cos(origx))
    Ngc[2] = -np.sin(satAzimuth)*np.cos(origy)

    # 2. calculate unit vector geocentric coordinates for lon/lat position (x,y)

    cartxy[:,0] = np.cos(x)*np.cos(y)
    cartxy[:,1] = np.sin(x)*np.cos(y)
    cartxy[:,2] = np.sin(y)

    # 3. Dot product between Ngc and cartxy gives angle 90 degrees bigger than what we want
...
if __name__ == '__main__':
    ...
    utmLoc   = Vector([locationE], [locationN], [0])
    refPoint = Vector(interferogram_refx, interferogram_refy, 0)

    LOS = getLOSvector(utmLoc, locationUTMzone, refPoint, satellite_altitude, satellite_azimuth, interferogram_ref_incidence, local_earth_radius)

    true = Vector([0.427051, -0.090772, 0.899660])
    print true, LOS
